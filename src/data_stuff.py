...
class normalizer(object):
    def __init__(self, X):
        ...
        self._X_mean_never_touch_dynamic = np.tile(X.mean(axis=1), (X.shape[1], 1)).T
        self._X_std_never_touch_dynamic = np.tile(X.std(axis=1)+1e-5, (X.shape[1], 1)).T
...
#        else:
#            #print("Normalized dim %s for NN model"%len(X))
        return (X-self._X_mean_never_touch_dynamic)/self._X_std_never_touch_dynamic, (Y-self._Y_mean_never_touch)/self._Y_std_never_touch
