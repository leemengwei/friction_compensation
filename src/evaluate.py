...
def evaluate_error_rate(args, outputs, targets, normer, raw_data, showup=False):
    ...
    value_threshold = 0.1*args.rated_torque
    ...
    error_rate_original = (np.abs(_plan[high_value_region] - _meassure[high_value_region])/np.abs(_meassure[high_value_region]+epsilon)).mean()*100
    error_rate_low = (np.abs(_plan[low_value_region] + _compensate[low_value_region] - _meassure[low_value_region])/np.abs(_meassure[low_value_region]+epsilon)).mean()*100
    error_rate_high = (np.abs(_plan[high_value_region] + _compensate[high_value_region] - _meassure[high_value_region])/np.abs(_meassure[high_value_region]+epsilon)).mean()*100
    #embed()
    if showup:
