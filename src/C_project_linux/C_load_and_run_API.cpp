...
int main(int argc, const char* argv[]) {
  ...
  for (int i = 0; i < LOOP; i++) {
    ...
    //Postprocess data:
    postprocess_data(out0);
    postprocess_data(out1);
    postprocess_data(out2);
    postprocess_data(out3);
    postprocess_data(out4);
    postprocess_data(out5);
    gettimeofday(&toc,0);
    double tictoc = 1000000*(toc.tv_sec-tic.tv_sec)+toc.tv_usec-tic.tv_usec; //us
    tictoc /= 1000; //ms
    running_t += tictoc;
