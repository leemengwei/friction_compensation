...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles, regularization_parameter=None, max_iter=None):
    ...
    pos_target = target_frame[:3, 3]
    angle_target = target_frame[:3, 0:3]
    pos_angle_target = target_frame[:3, :]
    ...
    def optimize_target(x):
        ...
        #squared_distance = np.linalg.norm(chain.forward_kinematics(y)[:3, 3] - pos_target)
        squared_distance = np.linalg.norm(chain.forward_kinematics(y)[:3, 0:3] - angle_target)
        #squared_distance = np.linalg.norm(chain.forward_kinematics(y)[:3, :] - pos_angle_target)
